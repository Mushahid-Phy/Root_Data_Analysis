...
void tut17() {
    ...
    for (int i = 0; i < 3; i++) {
        gr[i] = new TGraphErrors(Form("PbPb%d.dat", i)); // Adjust the file names accordingly
...
/*        //m0=0.9, 2.36, and 7 TeV
	...
	*/
        ...
        fEquation[i]->SetParLimits(3, 0.0001,0.200);
...
/*
    TLatex *u = new TLatex();
    u->SetTextSize(0.035);
    u->SetTextColor(kBlack);
    u->DrawLatex(0.5, 3.0, "Pb-Pb, #sqrt{s_{NN}} = 5.02 TeV");
    u = new TLatex(1.5, 1.0, "#Sigma(1385)^{+}, |y| <  0.5");
    u->Draw();
    u = new TLatex(0.5, 0.04, "VOM Multiplicity classes");
    u->Draw();
*/
